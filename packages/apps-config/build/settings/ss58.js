...
const networks = selectableNetworks.map(({
  displayName,
  network,
  prefix
}) => ({
  ...
  text: displayName,
  value: prefix
})).sort((a, b) => [0, 2, 42].includes(a.value) || [0, 2, 42].includes(b.value) ? 0 : a.text.localeCompare(b.text)); // Definitions here are with the following values -

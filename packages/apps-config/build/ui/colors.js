...
import { sanitize } from "./util.js"; // The mapping here is done on the actual chain name (system.chain RPC) or
...
export const chainColors = Object.entries({
  ...
  'ZERO.IO': chainZero
}).reduce((colors, [chain, color]) => ({ ...colors,
...
}).reduce((colors, [node, color]) => ({ ...colors,
  [sanitize(node)]: color
}), {}); // Alphabetical overrides based on the actual software node type
// NOTE: This is as retrieved via the system.name RPC
...
export const specColors = Object.entries({
  ...
  statemint: specStatemint,
  westmint: specWestmint
}).reduce((colors, [spec, color]) => ({ ...colors,
  [sanitize(spec)]: color
}), {});